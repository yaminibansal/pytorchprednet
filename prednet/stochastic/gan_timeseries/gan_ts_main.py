...
from prednet.stochastic.models.ts_models import StochLSTMFCEncDec
from prednet.stochastic.models.discriminators import cndtn_dcgan_dist, cndtn_dcgan_lstm
...
if opt.dataset == 'ball':
    ...
    f.close()
    X_train = np.swapaxes(np.swapaxes(data_container['videos'][:,0:4], 3, 4), 2, 3)
    Y_train = np.swapaxes(np.swapaxes(data_container['videos'][:,1:5], 3, 4), 2, 3)
    ...
    data_container = hkl.load(f)
    f.close()
    X_val = np.swapaxes(np.swapaxes(data_container['videos'][:,0:4], 3, 4), 2, 3)
    Y_val = np.swapaxes(np.swapaxes(data_container['videos'][:,1:5], 3, 4), 2, 3)
...
optimizerD = optim.Adam(disc.parameters(), lr=opt.dlr, betas=(opt.beta1, 0.999))
optimizerG = optim.Adam(gen.parameters(), lr=opt.glr, betas=(opt.beta1, 0.999))    
