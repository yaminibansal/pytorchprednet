...
class StochConvLSTMEncDec(nn.Module):
    def __init__(self):
...
class StochPredNet(nn.Module):
    def __init__(self, enc_filt_size, enc_ker_size, hid_filt_size, hid_ker_size, pool_enc_size, dec_ker_size, num_samples, num_dim_noise):
        '''
        Input Args:
        ...
        num_samples: Number of generated samples for each input datapoint
        num_dim_noise: Dimension of the input noise injected to topmost layer 
        ...
        self.num_dim_noise = num_dim_noise
    ...
    def forward(self, input, hidden, noise_append, noise=None):
        '''
        Input Args:
        noise: Noise sample of dimension batch_size x num_dim_noise 
        ...
        #Check if noise bathc size is equal to input
        if noise is not None:
            assert input.size()[0]==noise.size()[0]
        ...
        #Process the noise
        if noise_append==True:
            print(A[self.num_layers-1].size())
            noise_layer = noise.repeat(A[self.num_layers-1].size()[2], A[self.num_layers-1].size()[3], noise.size()[0], noise.size()[1])
            noise_layer = noise_layer.permute(2, 3, 0, 1)
        ...
        for layer in np.arange(self.num_layers-1, -1, -1):
            if layer == self.num_layers-1:
                if noise_append == True:
                    Rin[layer] = torch.cat((noise_layer, E[layer]), dim=1)
                else:
                    Rin[layer] = E[layer]
